...
const TopBarContainer = styled.div<{ isHome: boolean; visible: boolean }>`
  ...
  box-shadow: ${props => props.visible ? '0 2px 8px rgba(0, 0, 0, 0.05)' : 'none'};
`
...
const Content = styled.div<{ isHome: boolean }>`
  flex: 1;
  margin-bottom: 2rem;
  ${props => !props.isHome && 'margin-top: 5rem;'}
  width: 100%;
`
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setVisible(
        ...
        currentScrollPos < 10 || // Near the top
        (currentScrollPos + window.innerHeight) >= document.documentElement.scrollHeight // At bottom
      )
